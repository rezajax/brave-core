...
#include "brave/ios/browser/api/ui/webui/wallet_common_ui.h"
...
BraveWalletPageUI::BraveWalletPageUI(web::WebUIIOS* web_ui,
  ...
  web::WebUIIOSDataSource* source = web::WebUIIOSDataSource::Create(kWalletPageHost);
  ...
  if (url.path() == kWalletSwapPagePath) {
    source->AddResourcePaths(base::make_span(kBraveWalletSwapPageGenerated, kBraveWalletSwapPageGeneratedSize));
    source->SetDefaultResource(IDR_BRAVE_WALLET_SWAP_PAGE_HTML);
  } else if (url.path() == kWalletSendPagePath) {
    source->AddResourcePaths(base::make_span(kBraveWalletSendPageGenerated, kBraveWalletSendPageGeneratedSize));
    source->SetDefaultResource(IDR_BRAVE_WALLET_SEND_PAGE_HTML);
  } else if (url.path() == kWalletBuyPagePath) {
    source->AddResourcePaths(base::make_span(kBraveWalletFundWalletPageGenerated, kBraveWalletFundWalletPageGeneratedSize));
    source->SetDefaultResource(IDR_BRAVE_WALLET_FUND_WALLET_PAGE_HTML);
  } else if (url.path() == kWalletDepositPagePath) {
    source->AddResourcePaths(base::make_span(kBraveWalletDepositPageGenerated, kBraveWalletDepositPageGeneratedSize));
    source->SetDefaultResource(IDR_BRAVE_WALLET_DEPOSIT_PAGE_HTML);
  } else {
