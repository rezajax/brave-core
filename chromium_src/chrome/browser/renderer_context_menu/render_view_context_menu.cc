...
void BraveRenderViewContextMenu::ExecuteAIChatCommand(int command) {
  ...
  bool should_unlink_page_content = helper->ShouldUnlinkPageContent();
...
void BraveRenderViewContextMenu::InitMenu() {
  ...
  if (IsAIChatEnabled()) {
    ...
    if (inspect_index.has_value()) {
      menu_model_.InsertItemWithStringIdAt(inspect_index.value(),
                                           IDC_AI_CHAT_CONTEXT_SUMMARIZE_TEXT,
                                           IDS_AI_CHAT_CONTEXT_SUMMARIZE_TEXT);
    }
  }
#endif
}
