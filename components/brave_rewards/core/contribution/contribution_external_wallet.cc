...
void ContributionExternalWallet::Process(const std::string& contribution_id,
  ...
  engine_->database()->GetContributionInfo(
      contribution_id, ToLegacyCallback(base::BindOnce(
                           &ContributionExternalWallet::ContributionInfo,
  ...
  for (const auto& publisher : contribution->publishers) {
    ...
    engine_->publisher()->GetServerPublisherInfo(
        ...
        ToLegacyCallback(base::BindOnce(
            &ContributionExternalWallet::OnServerPublisherInfo,
            weak_factory_.GetWeakPtr(), contribution->contribution_id,
            publisher->total_amount, contribution->type,
            contribution->processor, single_publisher, std::move(callback))));
    return;
  }
...
void ContributionExternalWallet::OnServerPublisherInfo(
  ...
  auto start_callback = ToLegacyCallback(base::BindOnce(
      &ContributionExternalWallet::Completed, weak_factory_.GetWeakPtr(),
