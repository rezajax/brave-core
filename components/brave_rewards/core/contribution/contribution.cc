...
void Contribution::ProcessContributionQueue() {
  ...
  engine_->database()->GetFirstContributionQueue(
      ToLegacyCallback(base::BindOnce(&Contribution::OnProcessContributionQueue,
                                      weak_factory_.GetWeakPtr())));
}
...
void Contribution::CheckNotCompletedContributions() {
  engine_->database()->GetNotCompletedContributions(
      ToLegacyCallback(base::BindOnce(&Contribution::NotCompletedContributions,
                                      weak_factory_.GetWeakPtr())));
}
...
void Contribution::ContributionCompleted(
  ...
  engine_->database()->UpdateContributionInfoStepAndCount(
      ...
      -1,
      ToLegacyCallback(base::BindOnce(&Contribution::ContributionCompletedSaved,
                                      weak_factory_.GetWeakPtr(),
...
void Contribution::ContributionCompletedSaved(
  ...
  engine_->database()->MarkUnblindedTokensAsSpendable(
      contribution_id, ToLegacyCallback(base::BindOnce(
                           &Contribution::OnMarkUnblindedTokensAsSpendable,
...
void Contribution::MarkContributionQueueAsComplete(const std::string& id,
  ...
  engine_->database()->MarkContributionQueueAsComplete(
      id, ToLegacyCallback(
              base::BindOnce(&Contribution::OnMarkContributionQueueAsComplete,
                             weak_factory_.GetWeakPtr())));
}
...
void Contribution::CreateNewEntry(const std::string& wallet_type,
  ...
  engine_->database()->SaveContributionInfo(
      ...
      ToLegacyCallback(base::BindOnce(
          &Contribution::OnEntrySaved, weak_factory_.GetWeakPtr(),
          contribution->contribution_id, wallet_type, std::move(balance),
          std::move(queue))));
}
...
void Contribution::OnEntrySaved(const std::string& contribution_id,
  ...
  if (queue->amount > 0) {
    engine_->database()->SaveContributionQueue(
        queue->Clone(),
        ToLegacyCallback(base::BindOnce(&Contribution::OnQueueSaved,
                                        weak_factory_.GetWeakPtr(), wallet_type,
...
void Contribution::RetryUnblinded(
  ...
  engine_->database()->GetContributionInfo(
      contribution_id,
      ToLegacyCallback(base::BindOnce(&Contribution::RetryUnblindedContribution,
...
void Contribution::Result(const std::string& queue_id,
  ...
  engine_->database()->GetContributionInfo(
      contribution_id, ToLegacyCallback(base::BindOnce(
                           &Contribution::OnResult, weak_factory_.GetWeakPtr(),
                           result, queue_id)));
}
...
void Contribution::OnRetryTimerElapsed(const std::string& contribution_id) {
  ...
  engine_->database()->GetContributionInfo(
      contribution_id,
      ToLegacyCallback(base::BindOnce(&Contribution::SetRetryCounter,
                                      weak_factory_.GetWeakPtr())));
}
...
void Contribution::SetRetryCounter(mojom::ContributionInfoPtr contribution) {
  ...
  engine_->database()->UpdateContributionInfoStepAndCount(
      ...
      ToLegacyCallback(base::BindOnce(&Contribution::Retry,
                                      weak_factory_.GetWeakPtr(),
                                      std::move(contribution))));
}
...
void Contribution::GetRecurringTips(GetRecurringTipsCallback callback) {
  engine_->database()->GetRecurringTips(ToLegacyCallback(
