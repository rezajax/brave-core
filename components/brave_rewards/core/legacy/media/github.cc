...
void GitHub::ProcessActivityFromUrl(uint64_t window_id,
  ...
  engine_->database()->GetMediaPublisherInfo(
      media_key,
      ToLegacyCallback(base::BindOnce(&GitHub::OnMediaPublisherActivity,
...
void GitHub::GetPublisherPanelInfo(uint64_t window_id,
  ...
  engine_->database()->GetPanelPublisherInfo(
      std::move(filter),
      ToLegacyCallback(base::BindOnce(&GitHub::OnPublisherPanelInfo,
                                      weak_factory_.GetWeakPtr(), window_id,
                                      visit_data, publisher_key)));
}
...
void GitHub::OnMetaDataGet(GetPublisherInfoCallback callback,
  ...
  engine_->database()->GetMediaPublisherInfo(
      ...
      ToLegacyCallback(base::BindOnce(
          &GitHub::OnMediaPublisherInfo, weak_factory_.GetWeakPtr(), 0, user_id,
          user_name, publisher_name, profile_picture, std::move(callback))));
}
