...
void Promotion::Initialize() {
  if (!engine_->state()->GetPromotionCorruptedMigrated()) {
    ...
    engine_->database()->GetAllPromotions(ToLegacyCallback(base::BindOnce(
        &Promotion::CheckForCorrupted, weak_factory_.GetWeakPtr())));
  }

  engine_->database()->GetAllPromotions(ToLegacyCallback(
  ...
  for (const auto& item : list) {
    ...
    if (item->legacy_claimed) {
      ...
      engine_->database()->SavePromotion(
          item->Clone(), ToLegacyCallback(base::BindOnce(
                             &Promotion::LegacyClaimedSaved,
    ...
    engine_->database()->SavePromotion(
        item->Clone(), ToLegacyCallback(base::BindOnce([](mojom::Result) {})));
  }
  ...
  for (const auto& promotion : promotions) {
    ...
    if (!found) {
      engine_->database()->UpdatePromotionStatus(
          promotion.second->id, mojom::PromotionStatus::OVER,
          ToLegacyCallback(base::BindOnce([](mojom::Result) {})));
    }
  }
...
void Promotion::Claim(const std::string& promotion_id,
  ...
  engine_->database()->GetPromotion(
      promotion_id,
      ToLegacyCallback(base::BindOnce(&Promotion::OnClaimPromotion,
...
void Promotion::Attest(const std::string& promotion_id,
  ...
  engine_->database()->GetPromotion(
      promotion_id,
      ToLegacyCallback(base::BindOnce(&Promotion::OnAttestPromotion,
                                      weak_factory_.GetWeakPtr(),
...
void Promotion::OnAttestedPromotion(AttestPromotionCallback callback,
  ...
  engine_->database()->GetPromotion(
      promotion_id, ToLegacyCallback(base::BindOnce(
                        &Promotion::OnCompletedAttestation,
                        weak_factory_.GetWeakPtr(), std::move(callback))));
}
  ...
  engine_->database()->SavePromotion(
      ...
      ToLegacyCallback(
          base::BindOnce(&Promotion::AttestedSaved, weak_factory_.GetWeakPtr(),
                         std::move(callback), std::move(promotion))));
}
...
void Promotion::Complete(AttestPromotionCallback callback,
  ...
  engine_->database()->GetPromotion(
      promotion_id, ToLegacyCallback(base::BindOnce(
                        &Promotion::OnComplete, weak_factory_.GetWeakPtr(),
...
void Promotion::CheckForCorrupted(
  ...
  engine_->database()->UpdatePromotionsBlankPublicKey(
      ...
      ToLegacyCallback(base::BindOnce(&Promotion::CorruptedPromotionFixed,
                                      weak_factory_.GetWeakPtr())));
}
...
void Promotion::CorruptedPromotionFixed(mojom::Result result) {
  ...
  engine_->database()->GetAllCredsBatches(ToLegacyCallback(base::BindOnce(
...
void Promotion::CheckForCorruptedCreds(std::vector<mojom::CredsBatchPtr> list) {
  ...
  engine_->database()->GetPromotionList(
      corrupted_promotions,
      ToLegacyCallback(base::BindOnce(&Promotion::CorruptedPromotions,
                                      weak_factory_.GetWeakPtr(),
                                      corrupted_promotions)));
}
...
void Promotion::CorruptedPromotions(
  ...
  promotion_server_.post_clobbered_claims().Request(
      ...
      ToLegacyCallback(base::BindOnce(&Promotion::OnCheckForCorrupted,
                                      weak_factory_.GetWeakPtr(),
                                      std::move(ids))));
}
...
void Promotion::OnCheckForCorrupted(std::vector<std::string> promotion_id_list,
  ...
  engine_->database()->UpdatePromotionsStatus(
      promotion_id_list, mojom::PromotionStatus::CORRUPTED,
      ToLegacyCallback(base::BindOnce(&Promotion::ErrorStatusSaved,
                                      weak_factory_.GetWeakPtr(),
                                      std::move(promotion_id_list))));
}
...
void Promotion::ErrorStatusSaved(std::vector<std::string> promotion_id_list,
  ...
  engine_->database()->UpdateCredsBatchesStatus(
      ...
      ToLegacyCallback(base::BindOnce(&Promotion::ErrorCredsStatusSaved,
                                      weak_factory_.GetWeakPtr())));
}
